...
-- ═══════════════════════════════════════════
-- ЛОГГЕР (на каждый чих) + защита от повтора
-- ═══════════════════════════════════════════
...
-- ═══════════════════════════════════════════
-- ЛОАДИНГ-ЭКРАН (минимализм + анимации)
-- ═══════════════════════════════════════════
task.spawn(function()
...
-- Проверка поддержки Drawing (нужно для FOV, трассеров, 3D Box)
...
-- Кастомная фиолетовая тема Rayfield (полная, по схеме Rayfield 1.53+)
local ThemePurple = {
...
-- ═══════════════════════════════════════════
-- GUI-ОВЕРЛЕЙ (замена Drawing для Real, т.к. Drawing не отображается)
-- FOV, трассеры и 3D Box рисуются через GUI-объекты (Frame/UICorner)
-- ═══════════════════════════════════════════
...
-- Функция создания GUI-линии (возвращает frame + совместим с From/To/Visible/Color/Thickness)
...
-- Функция создания GUI-круга (рисуется сегментами-линиями, как ESP-трейсеры)
...
-- Fly переменные (оригинальный скрипт)
local speeds = 1
local nowe = false
...
-- Глобальная переменная ctrl для Fly управления
...
-- Анти-AFK функция (автоматически включается)
...
-- Автоматическое включение анти-AFK при запуске
ToggleAntiAFK()

-- WallHop функция (оригинальный скрипт WallHop V3)
...
local function performWallJump()
    ...
    if wallRayResult then
        ...
        -- === CONFIGURATION FOR MAIN JUMP ROTATION ===
...
local function ToggleWallhop()
    ...
    -- Обновляем кнопки мини-GUI
    updateMiniGuiButtons()
end

-- Бинд для переключения WallHop
...
-- ═══════════════════════════════════════════════════
-- МИНИ-МЕНЮ — мини-панель быстрого доступа
-- ═══════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════
-- SPEED BOOST С TRAIL-ПАРТИКЛАМИ (ТОГЛ)
-- ═══════════════════════════════════════════════════
...
-- Fly функция (оригинальный скрипт)
function ToggleFly()
    nowe = not nowe
...
-- Помечаем себя как пользователя ELITE HUB
pcall(function()
    ...
    player.CharacterAdded:Connect(function(char)
        ...
        -- Восстанавливаем CanCollide для всех частей
        local player = Players.LocalPlayer
        local character = player.Character
        if character then
            for _, part in pairs(character:GetDescendants()) do
...
local AimbotConfig = {
    ...
    -- Удержание цели до смерти/стены
    PersistentLock = true,
    -- ═══ Визуальные фичи ═══
    PulseTarget = false,
...
-- 3D FOV круг (GUI-версия, работает без Drawing)
...
-- Удержание цели: аимбот держится за одного врага, пока тот жив и на экране
local LockedTargetPlayer = nil

-- Предсказание: отслеживание предыдущих позиций
local PredictionLastPos = {}
local PredictionLastTime = {}

-- Kill notify: отслеживание HP целей
local PreviousTargetHP = {}

-- Уведомления: предотвращение спама (не чаще раза в 2 сек)
...
-- ===== СКЕЛЕТОН ИГРОКА (общие функции для ESP и Aimbot) =====
-- Два типа скелетона:
--   Тип 1: простой (общие связи R15, пропускает отсутствующие части).
--   Тип 2: полноценный (R15/R6, аккуратные соединения костей, как классический скелетон).

-- Тип 1: связи от A к B (R15-имена), пропускает отсутствующие.
...
-- Тип 2: соединения костей для R15 и R6.
...
-- Создаёт пул overlay-линий (хватает на любой тип скелетона)
...
-- Возвращает список связей и "имена" частей для выбранного типа и рига
...
-- Обновление скелетона: рисует линии по позициям костей персонажа.
...
-- ===== Индикатор текущей цели (стрелка + имя + HP + подсветка) =====
...
local function UpdateTargetIndicator()
    ...
    -- Стрелка указывает в сторону цели на экране
    ...
    -- Круг-подсветка вокруг цели на экране
    if onScreen then
...
-- ═══════════════════════════════════════════
-- СИСТЕМА ДРУЗЕЙ И ПРИОРИТЕТНОЙ ЦЕЛИ (по нику)
-- ═══════════════════════════════════════════
...
-- ═══════════════════════════════════════════
-- СИСТЕМА КОМАНД (Team): дружественные vs вражеские
-- список ДРУЖЕСТВЕННЫХ команд; остальные считаются врагами
-- хранится глобально -> работает и в Aimbot, и в ESP
-- ═══════════════════════════════════════════
getgenv().ELITE_HUB_FRIEND_TEAMS = getgenv().ELITE_HUB_FRIEND_TEAMS or {}

-- Имя команды игрока (по Player.Team / TeamColor); nil если нет команды
...
-- Принадлежит ли команда списку дружественных
...
-- Дружественная ли команда игрока (или он сам — всегда друг)
...
-- Дружественная ли команда игрока по отношению к ЕГО команде
-- (та же команда = друг, независимо от настроек)
...
-- Отношение к игроку: "my" (своя), "friend" (дружественная команда),
-- "enemy" (вражеская/нейтральная), "none" (без команды)
...
-- Собрать уникальные имена всех команд из текущих игроков (+своя)
...
-- Переключить команду в списке дружественных (вкл/выкл)
...
-- Авто-выбор части прицела: если голова видна -> в голову, иначе в видимую часть тела
...
local function TargetIsValid(targetPlayer)
    ...
    -- PersistentLock: держим цель пока она жива, видна (не за стеной) и на экране.
    -- Если цель ушла за стену или за кадр — отпускаем, чтобы аимбот переключился на другого.
...
local function GetClosestPlayer()
    ...
    -- 1) Держимся за уже заблокированную цель, пока она жива и на экране (не переключаемся)
    ...
    -- 2) Приоритетная цель по нику, если она задана
    ...
    -- 3) Ближайший враг в пределах FOV
    ...
    for _, targetPlayer in ipairs(Players:GetPlayers()) do
        ...
        if not skip then
            ...
            if not skip then
                -- Сначала дешёвые проверки (экран/дистанция/FOV), Рэйкаст (стены) — в конце
...
-- Основной цикл аимбота (защищён pcall, чтобы ошибка не убивала FOV-отрисовку)
task.spawn(function()
    ...
    while task.wait() do
        pcall(function()
            ...
            -- === Дистанция FOV: автоматическое изменение радиуса ===
            ...
            if Running and AimbotConfig.Enabled then
                ...
                if target then
                    ...
                    -- === Предсказание: учитываем скорость движения цели ===
...
-- Немедленно показываем вашу команду + реакция на смену команды
...
-- Автообновление лейбла вашей команды (Aimbot)
task.spawn(function()
    while true do
        task.wait(1)
        UpdateMyTeamLabel()
...
-- Обновление всех dropdown друзей/цели
...
-- Автообновление dropdown друзей (при входе/выходе игроков)
task.spawn(function()
    while true do
        task.wait(4)
        RefreshAimbotDD()
...
local ESPConfig = {
    ...
    HealthBarWidth = 4,
    HealthBarHeight = 40,
    -- Подсветка ближайшего врага (aimbot)
...
-- Создание стрелки-указателя для игрока (вне экрана)
...
-- Обновление стрелок: указываем направление на игроков вне экрана
            ...
            if pos then
                ...
                if not onScreen then
                    ...
                    if sv.Magnitude > 1 then
                        ...
                        -- Цвет стрелки зависит от команды
...
-- ===== Снап-лайны (линии от края экрана к игрокам, wallhack-стиль) =====
...
local function UpdateESPSkeletons()
    ...
    -- Скелетон показываем ТОЛЬКО на заблокированной цели аимбота.
    -- Для всех остальных игроков (и когда аимбот выключен/нет цели) — скрываем.
...
-- Бар HP под игроком (2 линии: фон + HP)
...
local function CreatePlayerESP(targetPlayer)
    ...
    -- Тег "ELITE HUB" над пользователями скрипта
...
-- Функция обновления 3D Box ESP (без continue + pcall защита)
...
-- Хранилище последних известных персонажей для отслеживания респауна
-- Хранилище последних известных персонажей (getgenv, чтобы не плодить local в main-скоупе)
getgenv().ELITE_HUB_LastChars = getgenv().ELITE_HUB_LastChars or {}

-- Полное обновление ESP (вызывается при изменении настроек — должен пересоздать всё)
...
-- Лёгкое обновление: пересоздаёт ESP только при смене персонажа (респаун/новый игрок)
...
-- Цикл обновления ESP (каждый кадр без задержки; создание/пересоздание — раз в 0.5с)
...
ESPTab:CreateToggle({
    ...
    Callback = function(value)
        ...
        if value then
            -- Автоматически включаем все функции ESP
...
-- Реакция на смену команды + немедленное отображение
...
-- Обновление всех dropdown друзей/цели (ESP)
...
-- Автообновление dropdown друзей (ESP)
task.spawn(function()
    while true do
        task.wait(4)
        RefreshESPDD()
...
-- ═══════════════════════════════════════════
-- ВИЗУАЛЬНЫЕ ФИЧИ: Прицел, Vignette, Пульс-цель, HP-бар цели
-- ═══════════════════════════════════════════
local VFX = {}
pcall(function()
...
-- === Кастомный прицел (удалён) ===

-- === Vignette при низком HP ===
...
-- === HP-бар заблокированной цели (над головой) ===
...
-- BillboardGui для HP-бара над головой цели
...
task.spawn(function()
    while task.wait(0.1) do
        pcall(function()
            ...
            -- HP-бар заблокированной цели (крепится к голове цели)
...
-- Цикл Kill All с увеличенной дистанцией до 10000 (без continue + pcall защита)
task.spawn(function()
...
-- ═══════════════════════════════════════════
-- ВИЗУАЛ: Небо, осадки, партиклы (как в Minecraft-читах)
-- ═══════════════════════════════════════════
local VisualConfig = {
    SkyEnabled = false,
    ...
    SkyBrightness = 1.0,
    SkyTime = 14,
    -- Заливка миром цветом (бывший "туман" от неба)
...
-- Определения типов частиц (как в китайских читах для Minecraft)
...
-- Настройка неба: цветное небо через ColorShift + Atmosphere
local function ApplyVisualSky()
    pcall(function()
    local L = VisFx.Lighting
    -- === Цветное небо ===
    if VisualConfig.SkyEnabled then
        -- Создаём/настраиваем Atmosphere - он красит горизонт и небо визуально
    ...
    -- === Заливка миром цветом (бывшее небо, выглядит как туман) ===
...
-- Создание частиц вокруг игрока (много типов)
...
-- Цикл автоматического применения неба и пересоздания частиц при респавне
task.spawn(function()
    player.CharacterAdded:Connect(function()
...
-- ─── NO FALL DAMAGE ─── (убрано по запросу)
